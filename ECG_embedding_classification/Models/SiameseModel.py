...
class Siamese(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1 = nn.Sequential(
            nn.Conv1d(num_features, num_features, kernel_size=kernel_size +
                      2, padding=(kernel_size // 2), stride=1),
            norm1d(num_features),
            like_LU_func(),
            nn.Dropout(dropout_rate),

            nn.Conv1d(num_features, num_features, kernel_size=kernel_size +
                      2, padding=(kernel_size // 2), stride=4),
            norm1d(num_features),
            like_LU_func(),
            nn.Dropout(dropout_rate)
        )
        ...
        self.conv2 = nn.Sequential(
            nn.Conv1d(num_features, num_features * 2, kernel_size=kernel_size +
                      2, padding=(kernel_size // 2), stride=1),
            norm1d(num_features * 2),
            like_LU_func(),
            nn.Dropout(dropout_rate),

            nn.Conv1d(num_features * 2, num_features * 2, kernel_size=kernel_size +
                      2, padding=(kernel_size // 2), stride=4),
            norm1d(num_features * 2),
            like_LU_func(),
            nn.Dropout(dropout_rate)
        ...
        self.conv4 = nn.Sequential(
            nn.Conv1d(num_features * 2, num_features * 3, kernel_size=kernel_size +
                      2, padding=(kernel_size // 2), stride=1),
            norm1d(num_features * 3),
            like_LU_func(),
            nn.Dropout(dropout_rate),

            nn.Conv1d(num_features * 3, num_features * 3, kernel_size=kernel_size +
                      2, padding=(kernel_size // 2), stride=4),
            norm1d(num_features * 3),
            like_LU_func(),
            nn.Dropout(dropout_rate)
